...
public class VoxelGrid : MonoBehaviour {
	...
	private float voxelSpacer = 1.0f;
	...
	private void InitializeVoxelColors() {
		for (int i = 0; i < voxels.Length; i++) {
			...
			Vector2 textureScale = new Vector2(1.0f / (float)voxelMaterials.Length, 1.0f / (float)voxelMaterials.Length);
						
			voxelMaterials[i].mainTextureOffset = new Vector2(1 - textureScale.x * i, 1 - (textureScale.y * (i + 1)));
			voxelMaterials[i].mainTextureScale = new Vector2(textureScale.x, textureScale.y);
